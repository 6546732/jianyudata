...
class OneDay:
    ...
    def configure(self):
        ...
        print(f'已设置 {self.day}、全国及四项匹配方式；四个关键词核对通过。', flush=True)
        self.filter_submit()
        # 仅进入订单预览；最终提交前再核对订单条数。
        WebDriverWait(self.driver, 25).until(lambda _: re.search(r'为您筛选到\s*\d+\s*条数据', self.body()))
    ...
    def execute(self):
        ...
        # 这一天已在本会话中多次确认成功，禁止默认重做付费步骤。
        if self.day == '2025-01-01':
            raise RuntimeError('2025-01-01 已在本次会话导出成功，请使用2025-01-02测试或下载1月1日原记录。')
...
def export_one_day(driver, day='2025-01-02', base=r'D:\桌面\data', confirm=True):
