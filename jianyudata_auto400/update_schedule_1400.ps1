...
foreach ($name in $times.Keys) {
    ...
    $daily = @($tasks[$name].Triggers | Where-Object { $_.CimClass.CimClassName -eq 'MSFT_TaskDailyTrigger' })
    if ($daily.Count -ne 1) { throw "任务 $name 必须有且仅有一个每日触发器。" }
}
$backup = Join-Path $env:LOCALAPPDATA ('JianyuTaskBackups\' + (Get-Date -Format 'yyyyMMddHHmmss'))
New-Item -ItemType Directory -Path $backup -Force | Out-Null
foreach ($name in $times.Keys) {
    Export-ScheduledTask -TaskName $name | Set-Content -LiteralPath (Join-Path $backup ($name + '.xml')) -Encoding utf8
}
$beijingDate = [DateTimeOffset]::UtcNow.ToOffset([TimeSpan]::FromHours(8)).ToString('yyyy-MM-dd')
foreach ($name in $times.Keys) {
