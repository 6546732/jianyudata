...
找不到按钮时，程序会先从页面顶部到底部滚动扫描。遇到页面遮挡、内容暂时缺失、标签页失效或自动化窗口断开时，程序会记录故障；遮挡场景先尝试点击经检测的安全空白处，仍失败时只重启经过核实的专用 Chrome 配置，最多进行10轮。找不到安全空白处就不点击。恢复记录保存在 `D:\桌面\data\logs\browser_retries.jsonl`；最后仍失败会发送本轮日志邮件。页面一旦存在待确认订单就立即停止，不重启浏览器或重复扣额。
