...
**最新入口请看 `README_FAST.md`：按省份字母顺序二分查找，默认100条测试上限。以下为之前的多日调度说明。**

## 2026-09-11 自动适配更新

- 默认采用 `AutomaticUI`，无需手输余额、省份或“查询完成”。
- 每天程序最多消耗200条（`daily_limit=200`），同时受网站实时余额约束。不会把昨天页面的余额当作今天额度。
- 余额通过新生成的订单预览读取；预览不点击扣除。省份按“全国清空 → 省份 → 全省 → 弹窗确定”选择，核对已选标签集合。
- 查询等待使用现场确认的 `/front/dataExport/sieveData` 请求完成事件，不读取旧结果，也不直接调用数据接口。
- 网站高级字段包有两行表头，现已支持识别；可验证并恢复旧 `submitted` 状态下已经下载的文件。
- 单省201～800条无法放进每天200条的配置时，返回 `needs_finer_split`，不会因此永久跳过或每天死循环。单省超过网站800条规则仍记录跳过。
- 默认不发送真实邮件，超额通知保存在待发送队列。需要发送时显式传入已配置的 `SMTPNotifier`。
- 1月1日和1月2日已在本次任务导出，当前续导从 `start='2025-01-03'` 开始，使用原 `base=r'D:\桌面\data'`。

```python
from selenium_range import export_range
state = export_range(driver, base=r'D:\桌面\data',
                     start='2025-01-03', confirm=True, daily_limit=200)
```

`InteractiveUI` 保留为可选手工模式，但不再默认启用。以下旧版说明中关于必须手工确认及必须配置SMTP的描述已由本节替代。
